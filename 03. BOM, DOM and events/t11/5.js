...
for (let i = 0; i < picArray.length; i++) {
    ...
    let elementFigureImg = document.createElement('img');
    let elementFigureCaption = document.createElement('figcaption');
    let elementP = document.createElement('p');

    elementArticle.appendChild(elementH2);
    elementArticle.appendChild(elementFigure);
    elementFigure.appendChild(elementFigureImg);
    elementFigure.appendChild(elementFigureCaption);
    elementArticle.appendChild(elementP);

    elementH2.textContent = picArray[i].title;
    elementFigureImg.src = picArray[i].image.medium;
    elementFigureCaption.textContent = picArray[i].caption;
    elementP.textContent = picArray[i].description;

    targetElement.appendChild(elementArticle);

    elementArticle.addEventListener('click', function () {
        let dialogElement = document.getElementById('dialogues');
        let dialogImage = dialogElement.querySelector('img');
        ...
        // Close the dialog when the span is clicked
        dialogElement.close();
    });
    });
